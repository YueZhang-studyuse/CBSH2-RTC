...
void ConstraintTable::insertDirectVertex2CT(size_t loc,int direct, int t_min, int t_max)
{
	assert(loc >= 0);
	direct_ct[loc*4+direct].emplace_back(t_min, t_max);
...
bool ConstraintTable::vertex_direct_constrained(size_t loc,int direct, int t) const
{
	assert(loc >= 0);

	const auto& it = direct_ct.find(loc*4+direct);
	if (it == direct_ct.end())
	{
		return false;
	}
...
void ConstraintTable::build(const CBSNode& node, int agent)
{
	...
	while (curr->parent != nullptr)
	{
		...
		case constraint_type::RANGE:
			...
			break;
		case constraint_type::DIRECT_VERTEX:
			if (a == agent)
			{
				for (const auto& constraint : curr->constraints) // we might have multiple vertex constraints generated by mutex propagation
				{
					tie(a, x, y, t, type) = constraint;
					insertDirectVertex2CT(x,y, t, t + 1);
				}
			}
			break;
		}
...
void ConstraintTable::buildCAT(int agent, const vector<Path*>& paths, size_t _cat_size)
{
	...
	if (map_size < map_size_threshold)
	{
		...
		//std::cout<<"agent num "<<paths.size()<<std::endl;
		for (size_t ag = 0; ag < paths.size(); ag++)
		{
			...
			{
			//std::cout<<"path size "<<paths[ag]->size()<<std::endl;
...
int ConstraintTable::getHoldingTime()
{
	...
	auto it_direct = direct_ct.find(goal_location*4 +goal_direction);
	if (it_direct != direct_ct.end())
	{
		for (auto time_range : it_direct->second)
			rst = max(rst, time_range.second);
	}
	return rst;
}
