...
void ConstraintTable::addConstraints(list<Constraint> cons, int agent)
{
	// for (auto con:cons)
	// {
		int a, x, y, t;
		constraint_type type;
		tie(a, x, y, t, type) = cons.front();
		switch (type)
		{
		case constraint_type::LEQLENGTH:
			...
			if ((int)cons.size() == 2) // generated by a corridor-target conflict
			{
				tie(a, x, y, t, type) = cons.back();
		...
		case constraint_type::VERTEX:
			if (a == agent)
			{
				for (const auto& constraint : cons) // we might have multiple vertex constraints generated by mutex propagation
				{
